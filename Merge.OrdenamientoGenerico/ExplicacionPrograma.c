...
tiene inicialmente mi vector. A esa cantidad la llamaré cant_ini. Y el vector será vec_prod.
Empiezo a hacer el merge y también allí usaré un vector (reg_nuevos) para acumular los nuevos registros, es decir,
los que figuran solo en movs.dat. También tengo un contador para estos registros (cant_nuevos). Lo necesitaré luego.
Una vez terminado el merge, cierro el archivo de movimientos y ya no lo vuelvo a usar. Pero sí usaré productos.dat
Utilizaré una función para abrir productos.dat y volcar todos los datos grabados y actualizados a un nuevo vector que
llamaré vec. Este vec tendrá una cantidad de registros total, es decir cant_ini + cant_nuevos.
No uso los dos vectores anteriores(vec_prod y reg_nuevos) ya que cada uno estaba seteado con una cantidad distinta,
y no podré asignarle mas registros de lo que permiten. Es por eso que utilizo este nuevo vector vec y además también
ordenaré en base a él, usando método de ordenamiento genérico con técnica de burbujeo.
Luego de ordenado, actualizamos el archivo.
Y finalmente preguntamos al usuario si desea completar los campos que quedaron vacíos.
